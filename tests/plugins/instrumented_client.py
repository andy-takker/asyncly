...
from asyncly import DEFAULT_TIMEOUT, ResponseHandlersType
...
class CatStruct(Struct):
    id: UUID
    name: str


class InstrumetedCatfactClient(InstrumentableHttpClient):
    ...
    CAT_HANDLERS: ResponseHandlersType = MappingProxyType(
        {
            HTTPStatus.OK: parse_struct(CatStruct),
        }
    )
    ...
    async def fetch_cat_by_id(
        ...
        return await self._make_req(
            method=hdrs.METH_GET,
            url=self._url / f"cats/{cat_id}",
            handlers=self.CAT_HANDLERS,
            timeout=timeout,
        )


@pytest.fixture
async def instrumented_client(
    catfact_url: URL,
