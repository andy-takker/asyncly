...
async def test_history_map_called_once(
    ...
    await catfact_client.fetch_msgspec_cat_fact()
    assert len(catfact_service.history_map["random_catfact"]) == 1
...
async def test_history_map_called_several_times(
    ...
    for _ in range(4):
        await catfact_client.fetch_msgspec_cat_fact()
    assert len(catfact_service.history_map["random_catfact"]) == 4
